...
-- Migrate existing valid data from bids to the new table
-- Only migrate records that have a valid user identifier
...
WHERE bank_account_number IS NOT NULL
  AND COALESCE(user_id, professional_id) IS NOT NULL
  AND NOT EXISTS (
    SELECT 1 FROM public.bid_payment_details d WHERE d.bid_id = bids.id
  );

-- Remove sensitive column from bids table to prevent exposure
